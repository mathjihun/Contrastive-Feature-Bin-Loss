...
#def CFBLoss(features, gt, features2, gt2, mean_centers, dataset='nyudepthv2', mask=None):
def CFBLoss(features, gt, mean_centers, dataset='nyudepthv2', mask=None):
    ...
    f_n, f_c, f_h, f_w = features.size()     # batch x 192 x 30 x 30
    ...
    gt = F.interpolate(gt.unsqueeze(dim=1), size=[f_h, f_w], mode='nearest')  # 2, 1, 480, 480
    #gt = F.interpolate(gt.unsqueeze(dim=1), size=[f_h, f_w], mode='nearest')      # 1 x 480 x 480 ->  1 x 1 x 480 x 480 -> 1 x 1 x 30 x 30 Siamese Version
    #gt2 = F.interpolate(gt2.unsqueeze(dim=1), size=[f_h, f_w], mode='nearest')
    
    '''
    # q0 style
    features = F.interpolate(features, size=[f_h//8, f_w//8], mode='nearest')
    ...
    gt = F.interpolate(gt, size=[f_h//8, f_w//8], mode='nearest')
    ...
    for i in range(f_n):
        ...
        batchwise_loss = _diverse_tightness(concat_features, concat_gt, f_c, centers)
        '''
