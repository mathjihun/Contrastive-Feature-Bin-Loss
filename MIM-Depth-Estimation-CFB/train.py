...
from utils.CFBLoss import CFBLoss as cfbloss
...
def main():
    ...
    model = GLPDepth(args=args, is_train=True)
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if global_step < iterations * half_epoch:
            current_lr = (args.max_lr - args.min_lr) * (
                global_step / iterations / half_epoch
            ) ** 0.9 + args.min_lr
        else:
            current_lr = max(
                args.min_lr,
                (args.min_lr - args.max_lr)
                * (global_step / iterations / half_epoch - 1) ** 0.9
                + args.max_lr,
            )

        for param_group in optimizer.param_groups:
            param_group["lr"] = (
                current_lr * param_group["lr_scale"]
                if "swin" in args.backbone
                else current_lr
            )
        ...
        optimizer.zero_grad()
        silog_loss = criterion[0](preds["pred_d"].squeeze(), depth_gt)

        # bin centers
        mean_centers = preds["centers"]
        centers_min_depth = 0.001 if args.dataset == "nyudepthv2" else 1.0
        mean_centers = torch.cat(
            [
                torch.full((mean_centers.size(0), 1, 1, 1), centers_min_depth).cuda(),
                mean_centers,
                torch.full((mean_centers.size(0), 1, 1, 1), args.max_depth).cuda(),
            ],
            dim=1,
        )

        # cfb loss
        cfb_loss = criterion[1](
            preds["feature"], depth_gt, mean_centers, dataset=args.dataset
        )

        loss_d = silog_loss + cfb_loss
        ...
        if batch_idx % args.print_freq == 0:
            result_line = "Epoch: [{0}][{1}/{2}]\t" "Loss: {loss}, LR: {lr}\n".format(
                epoch, batch_idx, iterations, loss=depth_loss.avg, lr=current_lr
            )
        ...
        if global_step % args.val_freq == 0:
            results_dict, loss_val = validate(
                val_loader,
                model,
                criterion[0],
                device=device,
            ...
            result_lines2 = logging.display_result(results_dict)
            if args.kitti_crop:
            ...
            with open(log_txt, "a") as txtfile:
                txtfile.write(
                    "\nEpoch: %03d - %03d, Global_step: %d"
                    % (epoch, args.epochs, global_step)
                )
                txtfile.write(result_lines2)

            for each_metric, each_results in results_dict.items():
                writer.add_scalar(each_metric, each_results, global_step)

            if args.save_model and epoch >= 10:
                torch.save(
                    {
                        "model": model.state_dict(),
                        "optimizer": optimizer.state_dict(),
                    ...
                    os.path.join(
                        log_dir, f"epoch_{epoch:02d}_{global_step}_model.ckpt"
                    ),
                )
...
def validate(val_loader, model, criterion_d, device, epoch, global_step, args):
    depth_loss = logging.AverageMeter()
    model.eval()
            ...
            if args.shift_window_test:
                ...
                for i in range(args.shift_size):
                    sliding_images.append(
                        input_RGB[..., :, i * interval : i * interval + h]
                    )
