...
class PixelFormer(nn.Module):
    ...
    def __init__(self, version=None, inv_depth=False, pretrained=None, 
        ...
        #self.feature_conv1 = nn.Conv2d(512, 128, 3, stride=1, padding=1)
        self.bcp = BCP(out_features=128, max_depth=max_depth, min_depth=min_depth)
        self.max_depth = max_depth
    ...
    def forward(self, imgs):
        ...
        f = self.disp_head1(q0, 4)
        f = f * self.max_depth

        if self.training:
            bin_centers = self.bcp(q4)
            # only nyu
            #q = F.interpolate(q4, scale_factor=2, mode='nearest')
            #q = self.feature_conv1(q)
            
            return f, q4, bin_centers
        else:
            return f
