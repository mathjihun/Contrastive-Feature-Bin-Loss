...
def eval(model, dataloader_eval, post_process=False):
    ...
    if not os.path.exists('eval.txt'):
        os.system('touch eval.txt')

    with open('eval.txt', 'a') as f:
        f.write('Computing errors for {} eval samples'.format(int(cnt)), ', post_process: ', post_process, '\n')
        f.write("{:>7}, {:>7}, {:>7}, {:>7}, {:>7}, {:>7}, {:>7}, {:>7}, {:>7}".format('silog', 'abs_rel', 'log10', 'rms',
                                                                                    'sq_rel', 'log_rms', 'd1', 'd2',
                                                                                    'd3') + '\n')
        for i in range(8):
            f.write('{:7.4f}, '.format(eval_measures_cpu[i]))
        f.write('{:7.4f}'.format(eval_measures_cpu[8]) + '\n')
        f.write('\n')
