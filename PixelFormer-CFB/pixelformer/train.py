...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = PixelFormer(version=args.encoder, inv_depth=False, max_depth=args.max_depth, pretrained=args.pretrain, training=True)
    model.train()
    ...
    #print(f"memory usage1: {torch.cuda.memory_allocated() / 1024**3:.2f} GB")
    #print(f"memory reserved1: {torch.cuda.memory_reserved() / 1024**3:.2f} GB")
    ...
    #print(f"memory usage2: {torch.cuda.memory_allocated() / 1024**3:.2f} GB")
    #print(f"memory reserved2: {torch.cuda.memory_reserved() / 1024**3:.2f} GB")

    group = dist.new_group([i for i in range(ngpus_per_node)])
    while epoch < args.num_epochs:
        #print(f"memory usage3: {torch.cuda.memory_allocated() / 1024**3:.2f} GB")
        #print(f"memory reserved3: {torch.cuda.memory_reserved() / 1024**3:.2f} GB")
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            depth_est, features, mean_centers = model(image)
            ...
            centers_min_depth = 0.1 if args.dataset == 'nyu' else 1.0

            # adaptive
            fixed_centers = torch.cat([torch.full((mean_centers.size(0), 1, 1, 1), centers_min_depth).cuda(), mean_centers, torch.full((mean_centers.size(0), 1, 1, 1), args.max_depth).cuda()], dim=1)

            '''            
            # uniform
            want_size = 1024
            fixed_centers = torch.linspace(start=centers_min_depth, end=args.max_depth, steps=want_size+2)
            fixed_centers = fixed_centers.repeat(features.size(0), 1)
            fixed_centers = fixed_centers[:, :, None, None].cuda()
            '''

            '''
            # log uniform
            want_size = 1024
            fixed_centers = torch.linspace(start=math.log(centers_min_depth), end=math.log(args.max_depth), steps=want_size+2)
            fixed_centers = torch.exp(fixed_centers)
            fixed_centers = fixed_centers.repeat(features.size(0), 1)
            fixed_centers = fixed_centers[:, :, None, None].cuda()
            '''
            ...
            # random
            want_size = 1024
            fixed_centers = torch.FloatTensor(want_size).uniform_(centers_min_depth, args.max_depth)
            fixed_centers = torch.sort(fixed_centers).values.unsqueeze(0).expand(image.size(0), -1)[:, :, None, None]
            fixed_centers = torch.cat([torch.full((image.size(0), 1, 1, 1), centers_min_depth).cuda(), fixed_centers.cuda(), torch.full((image.size(0), 1, 1, 1), args.max_depth).cuda()], dim=1)
            '''

            cfb = cfbloss(features, depth_gt, fixed_centers, dataset=args.dataset)

            silogloss = silog_criterion.forward(depth_est, depth_gt, mask.to(torch.bool))
            ...
            if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                ...
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                    ...
                    writer.add_scalar('silog_loss', silogloss, global_step)
                    writer.add_scalar('cfb_loss', cfb, global_step)
            ...
            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and epoch>=1 and not model_just_loaded:
                time.sleep(0.1)
                model.eval()
                with torch.no_grad():
                ...
                if eval_measures is not None:
                    for i in range(9):
                        ...
                        if is_best:
                            ...
                            checkpoint = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'global_step': global_step, 'epoch': epoch+1}
        ...
        if (epoch+1) % 5 == 0:
            checkpoint = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'global_step': global_step, 'epoch': epoch+1}
            torch.save(checkpoint, args.log_directory + '/' + args.model_name + f'/model_epoch_{epoch+1}')
        '''
